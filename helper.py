...
def generate_mock_results(ds_name, model_name, num_samples):
    ...
    baselines = {
        "Fashion-MNIST": {"PCA": (0.05, 9.46), "t-SNE": (18.62, 19.12), "Isomap": (0.98, 104.24),
                          "UMAP": (33.43, 79.65)},
    ...
    mock_labels = np.zeros(num_samples, dtype=int)  # Dummy fallback labels
...
def profile_model_performance(model_instance, X_eval, num_runs):
    """Profiles a model over multiple runs, averaging execution time and peak memory."""
    execution_times = []
    peak_memories = []
    X_reduced = None
